...
def segment_text(text: str, *, source_offset: int = 0, page_number: int = 0, final: bool = True) -> tuple[list[IngestedSentence], str, int]:
    ...
    if final and consumed < len(text):
        ...
        consumed = len(text)
    return sentences, text[consumed:], source_offset + consumed
        ...
        with source_path.open("r", encoding="utf-8", errors="replace", newline="") as stream:
            ...
            while True:
                ...
                if not chunk:
                    break
                buffer += chunk
                sentences, buffer, consumed_offset = segment_text(
                    buffer, source_offset=buffer_offset, final=False
                )
            ...
            sentences, buffer, consumed_offset = segment_text(
                buffer, source_offset=buffer_offset, final=True
            )
            self._persist_batch(
    ...
    def _ingest_pdf(
        ...
        for page_number, page_text in enumerate(iter_pdf_pages(source_path), start=1):
            ...
            if page_number <= start_page:
                ...
                continue
            sentences, _, _ = segment_text(
                page_text, source_offset=source_offset, page_number=page_number, final=True
            )
            source_offset += len(page_text)
            for start in range(0, len(sentences), self.batch_size):
                batch = sentences[start : start + self.batch_size]
