...
export function SentencesView({ client, project, capabilities, sentencePage, sentenceIsFetching, offset, selected, onOffset, onToggleSentence, narrationSidebarOpen, onToggleNarrationSidebar }: SentencesViewProps) {
  ...
  return <section className="sentence-workspace"><div className={`sentence-body ${narrationSidebarOpen ? "" : "narration-sidebar-closed"}`}><div className="sentence-main"><div className="sentence-summary" role="status" aria-live="polite"><span>{selected.length ? `${selected.length} selected` : "Select sentences to generate"}</span><span>{sentenceIsFetching ? "Updating…" : `Showing ${items.length ? offset + 1 : 0}–${Math.min(offset + 100, sentencePage?.total ?? 0)}`}</span></div>{sentenceIsFetching && !sentencePage ? <EmptyState loading title="Loading sentences" description="Retrieving sentence data…" /> : sentencePage && !items.length ? <EmptyState title="No sentences yet" description="Import a document to create sentences for narration." /> : <SentenceTable sentences={items} speakers={speakerItems} selectedIds={new Set(selected)} onToggle={onToggleSentence} loadAudio={(id) => client.audio(id)} onEdit={(sentence, text) => updateSentence.mutate({ id: sentence.id, changes: { text } })} onSpeaker={(sentence, speaker_id) => updateSentence.mutate({ id: sentence.id, changes: { speaker_id: speaker_id || null } })} />}<div className="pagination"><Button size="sm" variant="outline" disabled={offset === 0} onClick={() => onOffset(Math.max(0, offset - 100))}>Previous</Button><span>{offset + 1}–{Math.min(offset + 100, sentencePage?.total ?? 0)} of {sentencePage?.total ?? 0}</span><Button size="sm" variant="outline" disabled={offset + 100 >= (sentencePage?.total ?? 0)} onClick={() => onOffset(offset + 100)}>Next</Button></div></div>{narrationSidebarOpen && <NarrationSidebar speakers={speakerItems} profile={selectedProfile} capabilities={capabilities} selectedSpeakerId={selectedSpeakerId} onSpeaker={setSelectedSpeakerId} engine={engine} onEngine={(next) => { setEngine(next); setModel(""); setSettings({}); }} model={model} models={models} onModel={setModel} voice={voice} onVoice={setVoice} settings={settings} onSettings={setSettings} message={settingsMessage} saving={saveProfile.isPending} onSave={saveSettings} onToggle={onToggleNarrationSidebar} />}</div></section>;
}
