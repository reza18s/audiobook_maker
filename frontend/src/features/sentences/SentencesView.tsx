...
export function SentencesView({ client, project, capabilities, sentencePage, sentenceIsFetching, chapters, chapter, chapterIndex, onChapterIndex, selected, onToggleSentence, narrationSidebarOpen, onToggleNarrationSidebar }: SentencesViewProps) {
  ...
  const setSentenceStatus = useAppStore((state) => state.setSentenceStatus);
  ...
  return <section className="sentence-workspace"><div className={`sentence-body ${narrationSidebarOpen ? "" : "narration-sidebar-closed"}`}><div className="sentence-main"><div className="sentence-summary" role="status" aria-live="polite"><span>{selected.length ? `${selected.length} selected` : "Select sentences to generate"}</span><span>{sentenceIsFetching ? "Updating…" : chapter ? `${formatChapter(chapter)} · ${items.length.toLocaleString()} sentences` : "No chapters"}</span></div><div className="sentence-filter-bar"><Field label="Status"><Select value={sentenceStatus} onChange={(event) => setSentenceStatus(event.target.value)}><option value="">All statuses</option><option value="pending">Needs audio</option><option value="stale">Stale audio</option><option value="queued">Queued</option><option value="running">Generating</option><option value="completed">Generated</option><option value="failed">Failed</option><option value="cancelled">Cancelled</option></Select></Field>{sentenceStatus && <StatusBadge tone={sentenceStatus === "failed" ? "danger" : sentenceStatus === "completed" ? "success" : "info"}>Filtering: {sentenceStatus}</StatusBadge>}</div>{sentenceIsFetching && !sentencePage && !chapter ? <EmptyState loading title="Loading chapters" description="Retrieving chapter data…" /> : !chapters.length ? <EmptyState title="No sentences yet" description="Import a document to create chapters and sentences for narration." /> : sentencePage && !items.length ? <EmptyState title="No sentences in this chapter" description="This chapter has no sentences matching the current filters." /> : <SentenceTable sentences={items} speakers={speakerItems} selectedIds={new Set(selected)} onToggle={onToggleSentence} loadAudio={(id) => client.audio(id)} onEdit={(sentence, text) => updateSentence.mutate({ id: sentence.id, changes: { text } })} onSpeaker={(sentence, speaker_id) => updateSentence.mutate({ id: sentence.id, changes: { speaker_id: speaker_id || null } })} />}<div className="pagination"><Button size="sm" variant="outline" disabled={chapterIndex === 0 || sentenceIsFetching} onClick={() => onChapterIndex(Math.max(0, chapterIndex - 1))}>Previous chapter</Button><div className="pagination-label"><strong>{chapter ? formatChapter(chapter) : "No chapter selected"}</strong><span>{chapters.length ? `Chapter ${chapterIndex + 1} of ${chapters.length} · ${chapter?.sentence_count.toLocaleString() ?? 0} sentences` : "—"}</span></div><Button size="sm" variant="outline" disabled={chapterIndex >= chapters.length - 1 || sentenceIsFetching} onClick={() => onChapterIndex(Math.min(chapters.length - 1, chapterIndex + 1))}>Next chapter</Button></div></div>{narrationSidebarOpen && <button className="narration-drawer-backdrop" type="button" tabIndex={-1} aria-label="Close narration settings" onClick={onToggleNarrationSidebar} />}{narrationSidebarOpen && <NarrationSidebar speakers={speakerItems} profile={selectedProfile} capabilities={capabilities} selectedSpeakerId={selectedSpeakerId} onSpeaker={setSelectedSpeakerId} engine={engine} onEngine={(next) => { setEngine(next); setModel(""); setSettings({}); }} model={model} models={models} onModel={setModel} voice={voice} onVoice={setVoice} settings={settings} onSettings={setSettings} message={settingsMessage} saving={saveProfile.isPending} onSave={saveSettings} onToggle={onToggleNarrationSidebar} />}</div></section>;
}
