...
export function DocumentsView({ client, project }: { client: ApiClient; project: Project }) {
  ...
  const onFile = (file: File) => { setMessage(""); setUpload(0); uploadDocument.mutate(file); };
  ...
  return (
    <section>
      <PageHeader eyebrow="DOCUMENT IMPORT" title="Bring in a book" description="TXT and selectable-text PDF files are processed incrementally on the server." actions={<div className="document-import-actions"><TextField className="chapter-marker-field" label="Custom chapter marker" value={chapterMarker} onValueChange={setChapterMarker} placeholder="Optional line, e.g. [NEW CHAPTER]" helpText="Chapter headings and --- CHAPTER END --- are detected automatically." /><Field className="upload-button" label="Import document"><Input type="file" accept=".txt,.pdf,text/plain,application/pdf" disabled={uploadDocument.isPending} onChange={(event) => { const file = event.target.files?.[0]; if (file) void onFile(file); event.currentTarget.value = ""; }} /></Field></div>} />
