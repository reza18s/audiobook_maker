...
export function Topbar({ view, project, narrationSidebarOpen, onToggleNarrationSidebar, sentenceView, sentenceTotal, sentenceSpeakerCount, sentenceSelectedCount, sentenceQuery, onSentenceQuery, sentenceMoreOpen, onToggleSentenceMore, onCloseSentenceMore, onSelectAllSentences, onClearSentenceSelection, onRefreshSentences, onGenerateSentences, onDisconnect }: TopbarProps) {
  const pageTitle = view === "home" ? "Your projects" : view === "settings" ? "Settings" : view === "health" ? "Server health" : project?.name ?? "Project";
  return <header className={`topbar ${sentenceView ? "sentence-topbar" : ""}`}>{sentenceView && project ? <><div className="sentence-project-details"><span className="project-avatar large">{project.name.slice(0, 1).toUpperCase()}</span><div><div className="eyebrow">{project.document_count} DOCUMENTS · PROJECT</div><h1>{project.name}</h1><span>{sentenceTotal.toLocaleString()} sentences · {sentenceSpeakerCount.toLocaleString()} speakers</span></div></div><TopbarActions query={sentenceQuery} onQuery={onSentenceQuery} narrationSidebarOpen={narrationSidebarOpen} onToggleNarrationSidebar={onToggleNarrationSidebar} moreOpen={sentenceMoreOpen} onToggleMore={onToggleSentenceMore} onCloseMore={onCloseSentenceMore} onSelectAll={onSelectAllSentences} onClearSelection={onClearSentenceSelection} onRefresh={onRefreshSentences} selectedCount={sentenceSelectedCount} canSelectAll={Boolean(sentenceTotal)} onGenerate={onGenerateSentences} onDisconnect={onDisconnect} /></> : <><div className="topbar-leading"><div><div className="eyebrow">AUDIOBOOK MAKER</div><strong>{pageTitle}</strong></div></div><div className="topbar-actions"><span className="connection-status"><span className="status-dot" /> Gateway connected</span><button className="ghost" type="button" onClick={onDisconnect}>Disconnect</button></div></>}</header>;
}
...
export function TopbarActions({ query, onQuery, narrationSidebarOpen, onToggleNarrationSidebar, moreOpen, onToggleMore, onCloseMore, onSelectAll, onClearSelection, onRefresh, selectedCount, canSelectAll, onGenerate, onDisconnect }: TopbarActionsProps) {
...
export function WorkspaceSidebar({ open, onToggle, projects, project, view, tab, onHome, onProject, onTab, onSettings, onDisconnect }: WorkspaceSidebarProps) {
...
export function SidebarControlDock({ view, workspaceSidebarOpen, onToggleWorkspaceSidebar, settingsSidebarOpen, onToggleSettingsSidebar }: { view: AppView; workspaceSidebarOpen: boolean; onToggleWorkspaceSidebar: () => void; settingsSidebarOpen: boolean; onToggleSettingsSidebar: () => void }) {
  return <div className="sidebar-control-dock" role="group" aria-label="Sidebar controls"><SidebarToggle open={workspaceSidebarOpen} onClick={onToggleWorkspaceSidebar} label="navigation" controls="workspace-sidebar" />{view === "settings" && <SidebarToggle open={settingsSidebarOpen} onClick={onToggleSettingsSidebar} label="sections" controls="settings-sidebar" />}</div>;
}
