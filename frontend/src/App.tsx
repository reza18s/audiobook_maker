...
function ConnectedApp({ client, project, setProject, tab, setTab, view, setView, settingsTab, setSettingsTab, onDisconnect }: { client: ApiClient; project: Project | null; setProject: (project: Project | null) => void; tab: Tab; setTab: (tab: Tab) => void; view: AppView; setView: (view: AppView) => void; settingsTab: SettingsTab; setSettingsTab: (tab: SettingsTab) => void; onDisconnect: () => void }) {
  ...
  useEffect(() => { if (!sentenceMoreOpen) return; const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setSentenceMoreOpen(false); }; window.addEventListener("keydown", closeOnEscape); return () => window.removeEventListener("keydown", closeOnEscape); }, [sentenceMoreOpen]);
  ...
  return <div className={`app-shell ${workspaceSidebarOpen ? "" : "workspace-nav-collapsed"}`}><WorkspaceSidebar projects={projects.data ?? []} project={project} view={view} tab={tab} onHome={openHome} onProject={openProject} onTab={(next) => { setTab(next); setView(next === "health" ? "health" : "project"); }} onSettings={openSettings} onDisconnect={onDisconnect} /><div className="app-main"><header className={`topbar ${sentenceView ? "sentence-topbar" : ""}`}>{sentenceView && project ? <SentenceTopbar project={project} total={sentencePage.data?.total ?? 0} selectedCount={sentenceSelected.length} query={sentenceQuery} onQuery={(value) => { setSentenceOffset(0); setSentenceQuery(value); }} workspaceSidebarOpen={workspaceSidebarOpen} onToggleWorkspaceSidebar={() => setWorkspaceSidebarOpen((open) => !open)} narrationSidebarOpen={narrationSidebarOpen} onToggleNarrationSidebar={() => setNarrationSidebarOpen((open) => !open)} moreOpen={sentenceMoreOpen} onToggleMore={() => setSentenceMoreOpen((open) => !open)} onSelectAll={selectAllSentences} onClearSelection={clearSentenceSelection} onRefresh={refreshSentences} onGenerate={() => void queueSelectedSentences()} onDisconnect={onDisconnect} /> : <><div className="topbar-leading"><SidebarToggle open={workspaceSidebarOpen} onClick={() => setWorkspaceSidebarOpen((open) => !open)} label="navigation" /><div><div className="eyebrow">AUDIOBOOK MAKER</div><strong>{pageTitle}</strong></div></div><div className="topbar-actions">{view === "settings" && <SidebarToggle open={settingsSidebarOpen} onClick={() => setSettingsSidebarOpen((open) => !open)} label="sections" />}<span className="connection-status"><span className="status-dot" /> Gateway connected</span><button className="ghost" onClick={onDisconnect}>Disconnect</button></div></>}</header><main className={`content ${sentenceView ? "sentence-content" : ""}`}>{view === "home" && <HomeView projects={projects.data ?? []} onSelect={openProject} onCreate={(name) => createProject.mutate(name)} />}{view === "settings" && <SettingsView client={client} capabilities={capabilities.data ?? []} section={settingsTab} onSection={setSettingsTab} onBack={openHome} onDisconnect={onDisconnect} sidebarOpen={settingsSidebarOpen} />}{view === "health" && <HealthView client={client} />}{view === "project" && project && <ProjectWorkspace client={client} project={project} tab={tab} capabilities={capabilities.data ?? []} sentencePage={sentencePage.data} sentenceIsFetching={sentencePage.isFetching} sentenceQuery={sentenceQuery} sentenceOffset={sentenceOffset} sentenceSelected={sentenceSelected} onSentenceOffset={setSentenceOffset} onToggleSentence={(id) => setSentenceSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id])} narrationSidebarOpen={narrationSidebarOpen} onToggleNarrationSidebar={() => setNarrationSidebarOpen((open) => !open)} />}</main></div></div>;
}

function SidebarToggle({ open, onClick, label }: { open: boolean; onClick: () => void; label: string }) {
  return <button className="ghost sidebar-toggle" type="button" aria-label={`${open ? "Close" : "Open"} ${label} sidebar`} onClick={onClick}><span aria-hidden="true">{open ? "‹" : "›"}</span><span className="toggle-label">{open ? "Hide" : "Show"} {label}</span></button>;
}

function SentenceTopbar({ project, total, selectedCount, query, onQuery, workspaceSidebarOpen, onToggleWorkspaceSidebar, narrationSidebarOpen, onToggleNarrationSidebar, moreOpen, onToggleMore, onSelectAll, onClearSelection, onRefresh, onGenerate, onDisconnect }: { project: Project; total: number; selectedCount: number; query: string; onQuery: (value: string) => void; workspaceSidebarOpen: boolean; onToggleWorkspaceSidebar: () => void; narrationSidebarOpen: boolean; onToggleNarrationSidebar: () => void; moreOpen: boolean; onToggleMore: () => void; onSelectAll: () => void; onClearSelection: () => void; onRefresh: () => void; onGenerate: () => void; onDisconnect: () => void }) {
  return <><div className="topbar-leading"><SidebarToggle open={workspaceSidebarOpen} onClick={onToggleWorkspaceSidebar} label="navigation" /></div><div className="sentence-project-details"><span className="project-avatar large">{project.name.slice(0, 1).toUpperCase()}</span><div><div className="eyebrow">{project.document_count} DOCUMENTS · PROJECT</div><h1>{project.name}</h1><span>{total.toLocaleString()} sentences</span></div></div><div className="topbar-actions sentence-topbar-actions"><label className="search-field"><span>Search sentences</span><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="Search sentences…" /></label><SidebarToggle open={narrationSidebarOpen} onClick={onToggleNarrationSidebar} label="settings" /><div className="action-menu-wrap"><button className="ghost" type="button" aria-expanded={moreOpen} aria-haspopup="menu" onClick={onToggleMore}>More <span className="menu-chevron">⌄</span></button>{moreOpen && <div className="action-menu" role="menu"><button type="button" role="menuitem" onClick={onSelectAll}>Select all on page</button><button type="button" role="menuitem" onClick={onClearSelection}>Clear selection</button><button type="button" role="menuitem" onClick={onRefresh}>Refresh sentences</button></div>}</div><button className="primary" disabled={!selectedCount} onClick={onGenerate}>Generate {selectedCount || "selected"}</button><span className="connection-status compact-connection"><span className="status-dot" /> Connected</span><button className="ghost sentence-disconnect" onClick={onDisconnect}>Disconnect</button></div></>;
}
...
function ProjectWorkspace({ client, project, tab, capabilities, sentencePage, sentenceIsFetching, sentenceQuery, sentenceOffset, sentenceSelected, onSentenceOffset, onToggleSentence, narrationSidebarOpen, onToggleNarrationSidebar }: { client: ApiClient; project: Project; tab: Tab; capabilities: Capability[]; sentencePage?: SentencePage; sentenceIsFetching?: boolean; sentenceQuery?: string; sentenceOffset?: number; sentenceSelected?: string[]; onSentenceOffset?: (offset: number) => void; onToggleSentence?: (id: string) => void; narrationSidebarOpen?: boolean; onToggleNarrationSidebar?: () => void }) {
  return <>{tab === "documents" && <DocumentsView client={client} project={project} />}{tab === "sentences" && <SentencesView client={client} project={project} capabilities={capabilities} sentencePage={sentencePage} sentenceIsFetching={sentenceIsFetching ?? false} query={sentenceQuery ?? ""} offset={sentenceOffset ?? 0} selected={sentenceSelected ?? []} onOffset={onSentenceOffset ?? (() => undefined)} onToggleSentence={onToggleSentence ?? (() => undefined)} narrationSidebarOpen={narrationSidebarOpen ?? true} onToggleNarrationSidebar={onToggleNarrationSidebar ?? (() => undefined)} />}{tab === "speakers" && <SpeakersView client={client} project={project} capabilities={capabilities} />}{tab === "queue" && <QueueView client={client} />}{tab === "export" && <ExportView client={client} project={project} />}</>;
}
...
function SentencesView({ client, project, capabilities, sentencePage, sentenceIsFetching, query, offset, selected, onOffset, onToggleSentence, narrationSidebarOpen, onToggleNarrationSidebar }: { client: ApiClient; project: Project; capabilities: Capability[]; sentencePage?: SentencePage; sentenceIsFetching: boolean; query: string; offset: number; selected: string[]; onOffset: (offset: number) => void; onToggleSentence: (id: string) => void; narrationSidebarOpen: boolean; onToggleNarrationSidebar: () => void }) {
  const queryClient = useQueryClient();
  ...
  return <section className="sentence-workspace"><div className={`sentence-body ${narrationSidebarOpen ? "" : "narration-sidebar-closed"}`}><div className="sentence-main"><div className="sentence-summary"><span>{selected.length ? `${selected.length} selected` : "Select sentences to generate"}</span><span>{sentenceIsFetching ? "Updating…" : `Showing ${items.length ? offset + 1 : 0}–${Math.min(offset + 100, sentencePage?.total ?? 0)}`}</span></div><SentenceTable sentences={items} speakers={speakerItems} selectedIds={new Set(selected)} onToggle={onToggleSentence} loadAudio={(id) => client.audio(id)} onEdit={(sentence, text) => void update(sentence.id, { text })} onSpeaker={(sentence, speaker_id) => void update(sentence.id, { speaker_id: speaker_id || null })} /><div className="pagination"><button disabled={offset === 0} onClick={() => onOffset(Math.max(0, offset - 100))}>Previous</button><span>{offset + 1}–{Math.min(offset + 100, sentencePage?.total ?? 0)} of {sentencePage?.total ?? 0}</span><button disabled={offset + 100 >= (sentencePage?.total ?? 0)} onClick={() => onOffset(offset + 100)}>Next</button></div></div>{narrationSidebarOpen && <NarrationSettingsPanel speakers={speakerItems} profile={selectedProfile} capabilities={capabilities} selectedSpeakerId={selectedSpeakerId} onSpeaker={setSelectedSpeakerId} engine={engine} onEngine={(next) => { setEngine(next); setModel(""); setSettings({}); }} model={model} models={models} onModel={setModel} voice={voice} onVoice={setVoice} settings={settings} onSettings={setSettings} message={settingsMessage} saving={savingSettings} onSave={() => void saveSettings()} onToggle={onToggleNarrationSidebar} />}</div></section>;
}
